...
const CategoriesTable = ()=>{
	...
	const activateCategory = (selectedcategory)=>{
        ...
        <div>
			...
			<div className="flex fixed top-0 left-0 justify-center items-center w-full h-full bg-gray-600 bg-opacity-50">
				<div className="bg-white rounded shadow-lg p-[2rem] h-[35%] w-[30%] border">
					...
					<div className="mt-5">
						<button onClick={()=>{deleteCategory()}} className="bg-blue-500 mr-5 hover:bg-blue-700 text-sm text-white font-bold py-2 px-4 rounded mb-4 outline-none">Ok</button>
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
					...
					return (
						<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
							...
							<td className="px-6 py-4">
								<button  className="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2" onClick={()=>{activateCategory(item)}}><HiPencil/></button>
				...
				{updateCategory && <UpdateCategoryModal handleCatClose={closeUpdateCategory} category={category}/>}
        </div>
    )
};
